...
mypath = '/media/yara/Common/classes/bigData/HW/hw3data/'
onlyfiles = [ mypath + f for f in listdir( mypath) if isfile(join(mypath,f)) ]


lns = list(itertools.chain.from_iterable(map((lambda fl: open(fl, 'r') ), onlyfiles)))
# The data source can be any dictionary-like object
s.datasource = dict(enumerate(lns))
# s.datasource = dict(enumerate(open('../hw3data/x0155', 'r')))
s.mapfn = mapfn
s.reducefn = reducefn

rrresults = s.run_server(password="changeme")
print pprint.pprint(rrresults)
