...
Objects of the class \lstinline{Rational} have two \term{members},
...
Now define arithmetic functions. We can use \lstinline{Rational} as pure
datatype, and define operations outside:
...
\lstinline|(xs filter p, xs filterNot p)|, but computed in a single traversal of
the list \lstinline|xs|
...
\lstinline|(xs takeWhile p, xs dropWhile p)| but computed in a single traversal
...
More interesting - encoding from 
\lstinline|List(a, a, a, b, c, c, a)| to 
\lstinline|List(("a", 3),("b", 1),("c", 2),("a", 1))|
Here we go:
\begin{lstlisting}
...
\term{Reduction} of list is a process of combining elements - like sum, product etc.
Usual pattern - through \lstinline|reduceLeft| 
\begin{lstlisting}
...
\item Induction step: \lstinline|x :: xs|
For the left - hand side, we have:
\begin{lstlisting}
((x :: xs) ++ ys) ++ zs
...
\end{itemize}
{\bf Note} the \lstinline|:| always points to the sequence!
...
- \term{sequence} class. 
\lstinline|Seq| is he subclass of \lstinline|Iterable|, as well as
\lstinline|Set| and \lstinline|Map|
For diagram - see slides 6-1, page 4.  
...
\item \lstinline|xs.flatMap f| -  Applies collection-valued function \lstinline|f|
  to all elements of \lstinline|xs| and concatenates the results
...
\end{lstlisting}
(generally, the function value \lstinline|{ case p1 => e1 ... case pn => en }| is
equivalent to \lstinline|x => x match { case p1 => e1 ... case pn => en }|
...
\example Given a positive integer $n$, find all pairs of positive integers $i$
and $j$ with $ 1 \leq j < i < n$ such that $i + j$ is prime. 
